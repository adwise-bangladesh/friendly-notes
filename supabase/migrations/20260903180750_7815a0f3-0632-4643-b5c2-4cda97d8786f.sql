...
BEGIN
  ...
  -- Replay of the identical submission is a no-op, whatever the current state.
  IF _s.delivery_outcome_fingerprint = _fingerprint THEN
    RETURN _s;
  END IF;
  IF _s.delivery_outcome_fingerprint IS NOT NULL AND _s.delivery_outcome_recorded_at IS NOT NULL THEN
